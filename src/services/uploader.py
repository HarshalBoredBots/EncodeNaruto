...
class Uploader:
    ...
    async def upload(self):
        ...
        # destination_chat_id is set explicitly at task creation time.
        # Fall back to user_id only for tasks created before this field existed.
        destination = int(
            self.task_data.get("destination_chat_id") or user_id
        )
        output_file_name = self.task_data["output_filename"]
        send_type = self.task_data.get("send_type", "media")

        # ── Bot self-send protection ──────────────────────────────────────────
        bot_id = self.task_data.get("_bot_id")
        if bot_id and destination == int(bot_id):
            raise ValueError(
                f"[UPLOAD] Refusing self-upload destination "
                f"job={task_id} destination={destination} bot_id={bot_id}"
            )
